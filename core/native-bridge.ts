...
const convertBody = async (body: Document | XMLHttpRequestBodyInit | ReadableStream<any> | undefined): Promise<any> => {
...
const initBridge = (w: any): void => {
  const getPlatformId = (win: WindowCapacitor): 'android' | 'ios' | 'web' => {
    if (win?.androidBridge) {
      return 'android';
    } else if (win?.webkit?.messageHandlers?.bridge) {3
      return 'ios';
    } else {
      return 'web';
    }
      ...
      if (doPatchHttp) {
        ...
        window.fetch = async (
          ...
          try {
            ...
            const { data: requestData, type, headers: _headers } = await convertBody(options?.body || undefined);

            let headers = { ...(options?.headers ? options.headers : undefined), ..._headers };
